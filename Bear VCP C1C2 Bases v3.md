//@version=6
// ═══════════════════════════════════════════════════════════════════════════════
// Bear VCP C1C2 Bases v3 — v2 基础上【移除「主动创新低」规则】
//
...
// ═══════════════════════════════════════════════════════════════════════════════
indicator("Bear VCP C1C2 Bases v3", "BEAR C1C2 v3", overlay = true, max_boxes_count = 500, max_lines_count = 100, max_labels_count = 500)
...
findC1(int start, int n) =>
    ...
    for i = start to n - 1
        if lowIdx == -1 or cL(i) < cL(lowIdx)
            lowIdx := i   // 新低出现：低点后移(v2: 当根自己合格就算3连第一根)
            mAt = m50(i)
            lowUnder50 := not na(mAt) and cC(i) < mAt   // §4.2.3 前提：低点收盘在MA50下方
            streak := isCCandle(i) ? 1 : 0   // v2
            confirmed := false
            continue
        if isCCandle(i)
            streak += 1
...
if (barstate.islast or barstate.islastconfirmedhistory) and array.size(aC) != lastN
            ...
            while searchStart < n - 5
                ...
                while true
                    ...
                    if candLowIdx > confirmIdx - 2
                        ...
                        continue
                    // §4.3.3b 主动创新低规则已移除 (v3, 2026-07-21 用户指令)
                    if false
                        scanFrom := confirmIdx + 1
                        continue
    ...
    table.cell(infoT, 0, 0, "Bear VCP v3 — " + status + " | 窗口" + str.tostring(n) + "根已收盘", text_color = #B8860B, text_size = size.small, bgcolor = color.new(color.gray, 90))
