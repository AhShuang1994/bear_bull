//@version=6
// ═══════════════════════════════════════════════════════════════════════════════
// Bear VCP C1C2 Bases v2 — 试验版：3连确认的第一根可以是【低点当根本身】
//
...
// ═══════════════════════════════════════════════════════════════════════════════
indicator("Bear VCP C1C2 Bases v2", "BEAR C1C2 v2", overlay = true, max_boxes_count = 500, max_lines_count = 100, max_labels_count = 500)
...
if (barstate.islast or barstate.islastconfirmedhistory) and array.size(aC) != lastN
    ...
    table.cell(infoT, 0, 0, "Bear VCP v2 — " + status + " | 窗口" + str.tostring(n) + "根已收盘", text_color = #B8860B, text_size = size.small, bgcolor = color.new(color.gray, 90))
