//@version=6
// ═══════════════════════════════════════════════════════════════════════════════
// Bull VCP C1C2 Bases — plot_c1c2_bull.py 的 find_bases_bull() Pine 直译版
...
// ═══════════════════════════════════════════════════════════════════════════════
indicator("Bull VCP C1C2 Bases", "BULL C1C2", overlay = true, max_boxes_count = 500, max_lines_count = 100, max_labels_count = 500)
...
findC1(int start, int n) =>
    ...
    for i = start to n - 1
        if highIdx == -1 or cH(i) > cH(highIdx)
            highIdx := i   // 新高出现：高点后移，计数全部重来(当根不算回调)
            mAt = m50(i)
...
if (barstate.islast or barstate.islastconfirmedhistory) and array.size(aC) != lastN
            ...
            while searchStart < n - 5
                ...
                while true
                    ...
                    if candHighIdx > confirmIdx - 2
                        ...
                        continue
                    // §4.3.3b镜像 主动创新高：high 必须高于前一根 high(镜像SLX 885✓/SIREN 1893✗)
                    if candHighIdx > 0 and candHigh <= cH(candHighIdx - 1)
                        scanFrom := confirmIdx + 1
                        continue
    ...
    table.cell(infoT, 0, 0, "Bull VCP — " + status + " | 窗口" + str.tostring(n) + "根已收盘", text_color = #B8860B, text_size = size.small, bgcolor = color.new(color.gray, 90))
